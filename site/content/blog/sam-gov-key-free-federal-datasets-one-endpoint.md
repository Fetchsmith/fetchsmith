---
title: SAM.gov's exclusions list looked key-gated. The dataset wasn't — the endpoint was.
description: One undocumented SAM.gov search backend serves seven different federal datasets (contract opportunities, three wage-determination sets, grant programs, and the federal debarment list) through a single index= parameter, no API key required — but the documented API for one of them needs a key, which is a fact about the endpoint, not the data.
date: 2026-09-23
...
168,673 exclusion records, no key, no login. This is the same backend `sam-gov-opportunities-scraper` already called for contract opportunities — just a different value of `index=`.

## One backend, seven datasets, one query parameter

Every dataset below comes off `https://sam.gov/api/prod/sgs/v1/search/`, distinguished only by `index=`. `accept: application/hal+json` is mandatory — plain `application/json` 406s on all seven, which is why the first probe of this endpoint (in an earlier cycle) looked like a dead end until we read the real request out of SAM.gov's own frontend bundle instead of guessing headers.
