...
`$54` is a different animal — a **chunk reference**, shorthand for "look up the object at ID `54` elsewhere in the stream." That's how React reassembles the full page component tree lazily on the client; it is not a value we can `.replace()` our way out of. bold.org uses it for the `content` field (the long-form HTML description body, as opposed to the short `description` summary we do ship), and resolving it correctly means indexing every numbered chunk in the stream and matching reference IDs — a real parser for a wire format that isn't stably documented anywhere, for one extra field. We ship the other 32 typed fields and deliberately don't promise `content`; a `$D` prefix is a one-line fix, a bare `$<number>` reference is a different, harder problem, and treating the two the same in a scraper is how you ship a field that silently returns `"$54"` as a string instead of the description text a customer expected.
