...
The quirk (**updated 2026-09-10** — an earlier version of this paragraph blamed header fingerprinting, which we have since disproved): this feed is full of **holes**. For a given id/storefront/sort, some page numbers return a well-formed but empty feed while *later* pages return a full 50 — page 1 empty and page 5 full is common. It is sticky rather than random within a session (repeat the request, get the same empty result), which is exactly why it reads as an outage. On top of that, an **iPhone/iPad User-Agent gets a different, independently paginated index** over the same review pool — on one app the `curl` sweep and the iOS sweep returned 50 and 200 unique reviews with *zero* overlap. We [measured both mechanisms in detail for App Store app reviews](/blog/apple-app-store-reviews-header-fingerprint); podcast reviews ride the same infrastructure.

The fix is not "use a browser" and not a fingerprint rotation. It's to **scan the whole 1–10 page range and skip the holes** rather than `break` on the first empty page, dedupe by review id, and — if you want maximum coverage — sweep once with a default UA and once with an iOS Safari UA and union the results. Retrying the identical URL harder buys you nothing; looking at the pages you skipped buys you everything.
