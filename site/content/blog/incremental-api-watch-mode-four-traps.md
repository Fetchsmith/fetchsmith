...
We shipped this mode (`watchLabel`) across 17 Actors: six government-data hosts ([NIH RePORTER](/tools/nih-reporter-scraper), the [Federal Register](/tools/federal-register-scraper), [Grants.gov](/tools/grants-gov-scraper), [openFDA recalls](/tools/fda-recall-scraper), [ClinicalTrials.gov](/tools/clinicaltrials-scraper) and [court records](/tools/court-records-scraper)), two tender/procurement hosts ([EU TED](/tools/eu-ted-tenders-scraper) and [UK Find a Tender](/tools/uk-find-a-tender-scraper)), two money hosts ([US federal awards](/tools/us-federal-awards-scraper) and [FEC campaign finance](/tools/fec-campaign-finance-scraper)), a trademark registry aggregator ([TMview trademarks](/tools/trademark-search-scraper)), a forum ([Hacker News](/tools/hacker-news-scraper)), a job board aggregator ([ATS jobs](/tools/ats-jobs-scraper)) and four app/game/podcast platforms ([Google Play](/tools/google-play-reviews-scraper), [the Apple App Store](/tools/app-store-reviews-scraper), [Steam](/tools/steam-reviews-scraper) and [Apple Podcasts](/tools/apple-podcasts-scraper)). The state machine copied across all seventeen almost unchanged. What did not copy were the eight traps below. Each one was found on a *different* host, each produces a run that exits 0 with a cheerful log line, and each delivers either zero rows forever or a silent under-count.
...
Live on [Grants.gov](/tools/grants-gov-scraper), [openFDA recalls](/tools/fda-recall-scraper), [US federal awards](/tools/us-federal-awards-scraper), [ClinicalTrials.gov](/tools/clinicaltrials-scraper) and [NIH RePORTER](/tools/nih-reporter-scraper), off by default on all five. Verified the same way as everything else here — on the real platform, by mutating stored state rather than trusting a unit test: seed a baseline (509 opportunities; 29 Class I recalls; 26 awards; 481 studies; 23 projects), `PUT` two altered snapshots straight into the key-value store via the API, rerun, and confirm that exactly those two rows come back with the right tags and that the run charged for exactly two rows. One of the recalls came back as `D-0827-2026` status→`Terminated`, the other as `D-0832-2026` classification→`Class III`, which is precisely the email a recall analyst wanted and would never have received.
...
ClinicalTrials.gov was the fourth port, prompted by four small competitor Actors that appeared selling exactly "alert when a trial I already track changes status" as a separate paid product from "alert on new trials." The snapshot there is `overallStatus`, `lastUpdatePostDate`, `enrollmentCount`, `primaryCompletionDate` and `completionDate` — all fields the Actor already fetches for every study, so `watchChanges` adds zero new API calls. The one host-specific wrinkle: `rowsPerStudy:"site"` explodes a single study into one row per trial site, and the change tags have to propagate onto every exploded row while the snapshot itself stays tracked per-study, not per-row.
...
The fifth port, [NIH RePORTER](/tools/nih-reporter-scraper), is what that rule looks like when the answer comes back *true* — and it is worth spelling out, because the first thing we measured pointed the wrong way. A live `POST /v2/projects/search` shows that a non-competing renewal (`award_type: "5"`, `support_year` incrementing) gets a **brand-new `appl_id` every fiscal year**, which reads exactly like the append-per-event shape that had just disqualified the Federal Register. The premise only resolved by going to the host's own documentation rather than reasoning from the sample: eRA Commons' no-cost-extension help pages and NIH's NCE policy notices both state that an extension moves the project period end date **on the same award record**, no new application id minted. So an `appl_id` you were delivered last month genuinely can come back with a later `projectEndDate` — that is the NCE — and `watchChanges` snapshots `projectEndDate`, `budgetEnd`, `awardAmount` and `isActive`, all already fetched on a normal row.

The host-specific wrinkle here is on the *seeding* path, and it is a cost question rather than a correctness one. A baseline run over a fiscal year with 80,000+ projects is deliberately cheap: it asks NIH for only two fields (`ApplId`, `ProjectNum`) instead of the ~60-field full record. A snapshot needs more than that, so with `watchChanges` on, seeding requests six fields instead of two — still an order of magnitude lighter than the full record, but no longer free. Worth checking on any host where the incremental baseline is a deliberately thin projection: the snapshot fields have to be in that projection, or the first run after enabling the flag records `null` for all of them and the feature quietly starts a cycle late.
...
`watchLabel` is an optional input on 17 of our Actors — leave it unset and they behave exactly as before:
...
- [Trademark Search Scraper](/tools/trademark-search-scraper) — TMview's own `st13` id, keyed per search term/offices/classes/statuses; no `watchChanges` (a status moving `Filed` → `Registered` on a mark already delivered isn't tracked, only marks new to the baseline)
