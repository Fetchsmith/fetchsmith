...
| **Court Records & Case Law Scraper** | US federal court dockets (the free PACER/RECAP mirror) and published opinions from CourtListener's public API — parties, attorneys, law firms, assigned judge, nature of suit, filing entries, citations and cite counts. No API key or registration needed. | [Apify Store](https://apify.com/fetchsmith/court-records-scraper) · [Docs](https://fetchsmith.com/tools/court-records-scraper) · [Source](actors/court-records-scraper) |
| **Trademark Search Scraper** | Registered trademarks across 70+ national and regional offices (USPTO, EUIPO, UK, DE, JP, WIPO and more) via the official TMview database in one call: status, type, application/registration numbers and dates, applicant names, Nice classes, Vienna codes and mark image URL. | [Apify Store](https://apify.com/fetchsmith/trademark-search-scraper) · [Docs](https://fetchsmith.com/tools/trademark-search-scraper) · [Source](actors/trademark-search-scraper) |
| **SAM.gov Scraper** | Six public SAM.gov datasets in one Actor (`dataType`), no API key or login: contract opportunities (presolicitations, solicitations, sources sought, awards), Davis-Bacon/Service Contract Act/CBA wage determinations, CFDA grant/loan programs, and organization-only exclusion (debarment) records. Keyword, NAICS, set-aside, notice-type and state filters, plus optional detail enrichment for contracting-officer contacts, NAICS, set-aside and place of performance. | [Apify Store](https://apify.com/fetchsmith/sam-gov-opportunities-scraper) · [Docs](https://fetchsmith.com/tools/sam-gov-opportunities-scraper) · [Source](actors/sam-gov-opportunities-scraper) |
| **Remote Jobs Scraper** | Remote job postings from six public job boards — Remotive, Remote OK, Jobicy, Arbeitnow, Working Nomads and Himalayas — in one newest-first schema with no API key or login. De-duplicated across boards before charging, so a job syndicated to two sites is billed once and carries an `alsoOn` list; filter by keyword, company, location, salary presence and posted-date window, with parsed salary min/max/currency/period where the board publishes one. | [Apify Store](https://apify.com/fetchsmith/remote-jobs-scraper) · [Docs](https://fetchsmith.com/tools/remote-jobs-scraper) · [Source](actors/remote-jobs-scraper) |
| **SEC Insider Trades Scraper** | SEC EDGAR Form 3/4/5 insider ownership filings, one flat row per reported transaction (not a filing index): insider name/role, all 17 transaction codes decoded, shares, price, a signed pre-computed `transactionValueUsd`, shares held after, Rule 10b5-1 flag and footnotes. No personal address ever emitted. | [Apify Store](https://apify.com/fetchsmith/sec-insider-trades-scraper) · [Docs](https://fetchsmith.com/tools/sec-insider-trades-scraper) · [Source](actors/sec-insider-trades-scraper) |
...
- [HN search caps every query at 1,000 hits, no matter what nbHits claims](https://fetchsmith.com/blog/hacker-news-1000-hit-search-ceiling)
...
- [Grants.gov's search API has two opposite silent failures — and only one of them is safe](https://fetchsmith.com/blog/grants-gov-api-fails-open-and-closed)
...
- [Eight ways an "only new since last run" watch mode silently stops working](https://fetchsmith.com/blog/incremental-api-watch-mode-four-traps)
