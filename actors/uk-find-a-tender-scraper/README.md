...
**Do `contract` and `implementation` stages work the same way on both portals?** Not quite, though the result you get is correct either way. Contracts Finder's `stages` filter is a real server-side validator (it 400s on a bad value naming it) and, probed directly, accepts 5 OCDS stages — `planning`/`tender`/`award`/`contract`/`implementation` — all with genuine data. Find a Tender's own filter only ever recognized 3 of those (`planning`/`tender`/`award`); asking it for `contract` or `implementation` returns 0 rows with no error, even though FTS's own data does contain contract- and implementation-tagged notices — the same "silently matches nothing" trap as the multi-value bug above, just for a different set of values. Since v0.1.37, whenever `stages` includes `contract` or `implementation`, the Actor stops sending Find a Tender a `stages` filter at all (fetching every stage from that portal) and applies the stage filter itself client-side instead — the only way to actually reach that data. Contracts Finder is unaffected; its own filter already handles all 5 values correctly.
