...
| `watchLabel` | string | Turn the run into a **catalog watch**. The first run under a label records every product's price, availability and sale status and returns nothing (charges nothing); later runs under the same label return only products that are new, changed price, started/ended a sale, changed stock status, or have been removed from the catalog. See *Watch mode* below. |
| `watchEvents` | array | Restrict a watch to some change types only: `new`, `priceDrop`, `priceIncrease`, `wentOnSale`, `saleEnded`, `backInStock`, `outOfStock`, `delisted`. Empty (default) reports all eight. `wentOnSale`/`saleEnded` fire only when a compare-at price is added/removed with the current price unchanged — a price move that also flips the sale flag is reported as `priceDrop`/`priceIncrease` instead (the row's `previousIsOnSale` still shows the sale-state change). Ignored without `watchLabel`. |
| `webhookUrl` | string | Optional http(s) URL that receives a small JSON POST when the run finishes (products pushed, per-change-type counts, dataset id, and a per-URL `sources` outcome list — see the FAQ). Best-effort — a failing webhook never fails the run or changes the bill. |
...
```
`barcode`/`inventoryQuantity`/`inventoryManagement`/`inventoryTracked`/`inventoryPolicy`/`quantityRule` (per variant), `totalInventory`/`hasSubscriptionOption`/`subscriptionPlans` (per product), `seoTitle`, `seoDescription`, `ratingValue` and `reviewCount` are only present when `detailLevel` is `"full"` (see Input) — Shopify's bulk `products.json` feed carries none of them.
...
- **Run 1** records each product's price, availability and sale status and returns **0 products, charged $0** — it's a baseline.
- **Every run after that** returns only the products that moved. Each row is a normal product row plus `watchChange` (`new` / `priceDrop` / `priceIncrease` / `wentOnSale` / `saleEnded` / `backInStock` / `outOfStock` / `delisted`), `watchChanges` (all changes on that product), `previousPriceMin`, `previousAvailable`, `previousIsOnSale` and `priceChange` (the signed difference).
...
- A delisted row carries `id`, `handle`, `url`, `store`, `previousPriceMin`, `previousAvailable` and `previousIsOnSale`; the product-detail fields are `null`, because there is no longer a product to read them from. The `url` is the product's old URL — it 404s by definition, and is there to identify the product.
...
**Can I tell which products are on sale?** Yes, each item includes `isOnSale` (true when **some single variant** has its own compare-at price above its own price) and `discountPercent` — the percentage off the cheapest variant's own list price, so it never mixes two different variants — plus store `currency`. Both are deliberately per-variant: comparing `compareAtPriceMin` against `priceMin` reads two numbers that can belong to different variants, so a stale compare-at left below a raised price can report a sale no variant is actually running.
...
**Can I get real inventory counts and barcodes?** Yes, set `detailLevel: "full"`. Shopify's bulk `products.json` feed strips `barcode`, `inventory_quantity`, `inventory_management` and `inventory_policy` out of every variant, but the per-product JSON route still serves them on the same variant ids, so they merge back in exactly. How much a given store exposes is that store's own setting (see the note under Output) — you get real values where they are published and `null` where they are not, never a guess. No login or Admin API token is involved; this is the same data a logged-out shopper's browser receives. Every variant also gets `inventoryTracked` (`true`/`false`) so "this quantity is real" is a named field rather than something you infer from `inventoryManagement` being non-null — untracked variants report a 999999 sentinel quantity, and that trap is easy to walk into once you flatten `variants[]` into a CSV and drop a column. It is `null`, not `false`, on rows that carry no inventory data at all (`detailLevel: "basic"`), because "not tracked" and "not asked" are different answers.
...
**What happens if a store's storefront has a transient connection blip mid-scan?** Every request is retried up to 3 times on a connection-level failure (measured at roughly 1 fresh request in 4 for HTTP/2 faults, 2026-09-21) before that store is marked errored in the status message — a single blip no longer silently truncates a multi-page store to whichever page it reached.
...
**I automate this — how do I tell those cases apart in code, not by reading the log?** Every run writes a `RUN_SUMMARY` record to its own key-value store (`GET https://api.apify.com/v2/actor-runs/<runId>/key-value-store/records/RUN_SUMMARY`), and the same object is included in the `webhookUrl` POST as `sources`. It holds one entry per URL you passed, with `status` — `ok`, `empty`, `filteredOut`, `duplicate`, `error`, `badUrl`, `watchBaselined`, `watchNoChanges` or `notReached` — plus `scanned`, `delivered`, `duplicates`, `complete` and a `reason` string. `scanned` and `delivered` are separate fields on purpose: an empty dataset stops being ambiguous, and a store that errors today and succeeds tomorrow does not look like a catalog that suddenly appeared. `notReached` specifically means the run stopped (time limit, `maxResults`, charge budget) before that URL was fetched at all — never confuse it with an empty store.

**Just tell me if the run got everything — one field.** The same `RUN_SUMMARY` record carries run-level `complete`, `incompleteReason`, `incompleteDetail` and `sourcesIncomplete` alongside `sources`. `complete` is `true` only when every URL you passed was read to the end *and* the run did not stop on the clock; `false` otherwise; `null` if no URL was processed at all. `incompleteReason` names the first shortfall — `max-products-per-store`, `max-results`, `time-budget`, `charge-limit`, `source-request-failed`, `bad-url`, `not-reached`, `empty-source`, `filtered-out`, `duplicate-source`, `watch-baselined` or `watch-no-changes` — and `incompleteDetail` says which URL and how many others. Note that `complete` is separate from `status` on purpose: a URL with `status: "ok"` is still incomplete if `maxProductsPerStore` (default **500**) cut its sweep short, which is the most common reason a large catalog comes back partial. This is the same `complete` / `incompleteReason` / `incompleteDetail` contract every FetchSmith Actor's `RUN_SUMMARY` carries, so one pipeline can check all of them the same way.
...
**Can I monitor a store for price drops instead of re-scraping the whole catalog?** Yes — set `watchLabel` (optionally narrowed with `watchEvents`). The first run under that label is a free baseline that returns nothing; every later run returns only new products, price changes, sale starts/ends, stock flips and products that have disappeared from the catalog (`delisted`), each row carrying `watchChange`, `previousPriceMin`, `previousAvailable`, `previousIsOnSale` and `priceChange`. Products that didn't move are never pushed and never charged, so a scheduled daily watch costs nothing on days when the store doesn't change. See *Watch mode* above.

**Does the watch catch a markdown that doesn't change the listed price?** Yes — `wentOnSale`/`saleEnded` fire when a store adds or removes a compare-at ("was") price while the current price itself stays put, which `priceDrop`/`priceIncrease` alone would miss entirely. If the current price changes too, that's reported as `priceDrop`/`priceIncrease` instead (one event per row, not two) and the row's `previousIsOnSale` still shows whether the sale flag itself flipped.
