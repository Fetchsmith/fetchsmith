...
- [Four ways an "only new since last run" watch mode silently stops working](https://fetchsmith.com/blog/incremental-api-watch-mode-four-traps) — how `watchLabel` is built, why `newly_added_projects_only` is not the same thing, and the three-run test that proves a baseline is complete.
