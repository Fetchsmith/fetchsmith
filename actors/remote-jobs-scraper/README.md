...
| [Himalayas](https://himalayas.app) | By far the largest board here — **~102,000 live postings** per its own API (measured 2026-09-23), cursor-paginated 20 at a time, so `maxPagesPerSource` caps how deep a run walks into it | numeric min/max/currency/period on most rows |
...
- Remote OK's and Jobicy's numbers are rendered into `salaryText` (`"$250,000 - $315,000 per year"`, `"Up to $127,000"`). The `per year` suffix appears only where the board stated the period — Jobicy sends one, Remote OK does not. Jobicy also states a currency, Remote OK does not, so Remote OK rows read bare digits: `"250,000 - 315,000"`, no `$`.
