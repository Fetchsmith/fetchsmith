...
**How do I know whether a run hit that 500 ceiling or simply collected every review there is?** The run tells you, as of v0.1.49. When Apple's last servable page comes back full and the Actor was still taking reviews, the run's status message names the app/storefront pairs that hit the ceiling and says plainly that raising `maxReviewsPerApp` cannot reach the older reviews (widen `countries`, or schedule with `watchMode` instead). A run that silently ends below the ceiling genuinely exhausted what Apple serves for that pair — so a *missing* ceiling note is itself the "you got everything available" signal.
