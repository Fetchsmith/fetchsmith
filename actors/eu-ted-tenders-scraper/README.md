...
| `noticeTypes` | array | TED notice-type codes, picked from a dropdown of **all 22 codes TED accepts** (e.g. `cn-standard` contract notice, `can-standard` award notice, `pin-only`/`pin-buyer`/`pin-rtl` prior information, `veat`, `corr`, `can-modif`, `qu-sy`, `pmc`). Empty = all. There is no `pin-standard` code — see the FAQ. |
...
**Which notice-type and procedure-type codes actually exist?** All of them are in the two dropdowns — we exhausted TED's own search index to get the complete lists rather than documenting a handful of examples: **22 notice types** (excluding all 22 leaves exactly 0 notices across TED's entire history) and **17 procedure types** (8 modern eForms codes plus 9 legacy pre-2023 single-character codes TED still accepts). Two upstream traps this closes: there is **no `pin-standard` code** (prior-information notices are split across six `pin-*` codes) and **no `exp-int-rest` procedure type** — both are plausible-looking values TED rejects outright with HTTP 400, which used to fail the run with a misleading "TED outage, please re-run" message. One asymmetry remains and it is TED's, not ours: the value `7` appears in the output `procedureType` field on some older notices, but TED's expert search refuses `7` as a *filter* value, so it is not selectable.
