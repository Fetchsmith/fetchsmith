...
### Location
`location` is always the board's own text. `city`, `region` and `country` are the normalised breakdown, and both `locationKeyword` and `locationExcludeKeyword` match against all four — so `locationExcludeKeyword: "United States"` now drops a Greenhouse posting whose text only reads `"San Francisco, CA"`.
