...
| `topics` | array | Optional: browse Google News' built-in sections without knowing an RSS URL — `WORLD`, `NATION`, `BUSINESS`, `TECHNOLOGY`, `ENTERTAINMENT`, `SCIENCE`, `SPORTS`, `HEALTH`, plus the narrower sub-sections `POLITICS`, `ECONOMY`, `REAL_ESTATE`, `JOBS`, `EDUCATION`, `AUTOS`, `MOVIES`, `MUSIC`, `CELEBRITIES`, `ARTS`, `SOCCER`, `BASKETBALL` |
